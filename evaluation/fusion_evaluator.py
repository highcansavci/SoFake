...
class FusionEvaluator:
    ...
    Stage 2 – Signal analysis:
        rppg_score  = cardiac-band power / total signal power  in [0, 1]
                      High  -> strong regular cardiac signal (likely real face)
                      Low   -> weak / absent signal  (may indicate synthetic)
        fft_score   = peak heart-rate confidence derived from FFT PSD in [0, 1]
                      Normalised so that 40 bpm -> 0.0 and 200 bpm -> 1.0
    """

    _CLIP_LEN = 30    # frames per inference window
    _STRIDE   = 15    # how often (in frames) to run inference
    ...
    def __init__(self, video_path: str, weight_path: str):
        ...
        # ── Fix #1 & #3: load model_weights.pt into PhysNet (strict=True) ──
        self.physnet = PhysNet(S=2).to(self.device)
        state_dict   = torch.load(weight_path, map_location=self.device)
        self.physnet.load_state_dict(state_dict)   # strict=True by default
        self.physnet.eval()
    ...
    def _preprocess(self, frame: np.ndarray) -> np.ndarray:
        ...
        # Fix #2: BGR -> RGB color-space conversion
    ...
    def _to_tensor(self, frames: list) -> torch.Tensor:
        ...
        # Fix #3: build the correct 5-D input for PhysNet
    ...
    def _rppg_score(self, signal: np.ndarray, fps: float) -> float:
        """Cardiac-band power ratio in [0, 1]."""
        try:
            filtered    = butter_bandpass(signal, lowcut=0.6, highcut=4.0, fs=fps)
            total_power = float(np.var(signal))
            if total_power < 1e-8:
                return 0.0
            band_power = float(np.var(filtered))
            return float(np.clip(band_power / total_power, 0.0, 1.0))
        except Exception:
            return 0.0

    def _fft_score(self, signal: np.ndarray, fps: float) -> tuple:
        """FFT peak-HR confidence in [0, 1] and raw HR in bpm."""
        try:
            filtered        = butter_bandpass(signal, lowcut=0.6, highcut=4.0, fs=fps)
            hr_bpm, _, _    = hr_fft(filtered, fs=fps)
            # Normalise physiological range [40, 200] bpm -> [0, 1]
    ...
    def evaluate(self) -> list:
        ...
        buffer    = deque(maxlen=self._CLIP_LEN)
        results   = []
        frame_cnt = 0
        ...
        while True:
            ...
            buffer.append(self._preprocess(frame))
            frame_cnt += 1

            if len(buffer) == self._CLIP_LEN and frame_cnt % self._STRIDE == 0:
                # ── Stage 1: PhysNet 3D-CNN inference ─────────────────────
                clip = self._to_tensor(list(buffer))   # (1, 3, 30, 128, 128)
                with torch.no_grad():
                ...
                f_score, hr_bpm  = self._fft_score(signal, fps)
